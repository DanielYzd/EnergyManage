    ...
    >
      ...
      <el-form-item label="复位方式" v-if="showResetType">
        <el-radio-group v-model="dataForm.resetType">
          <el-radio
            style="display:block;height:35px;margin-left: 30px;"
      ...
      <el-form-item label="时间" v-if="dataForm.fn === '0531'">
        <el-input
          v-model="dataForm.rtuDate"
          placeholder="时间格式:2018-10-01 01:00:32"
          style="width: 80%;"
        ></el-input>
        <el-tooltip effect="dark" placement="top-start">
          ...
          <el-button
            @click="
              () => {
                dataForm.rtuDate = this.formatDate(new Date())
              }
            "
            icon="el-icon-time"
          ></el-button>
...
export default {
  ...
  computed: {
    showResetType() {
      return (
        this.dataForm.fn === '0101' ||
        this.dataForm.fn === '0102' ||
        this.dataForm.fn === '0103' ||
        this.dataForm.fn === '0104'
      )
    }
  },
  data() {
    return {
      ...
      rtuAddrs: [],
      resetTypes: [
        { key: '0101', label: '硬件初始化' },
        { key: '0102', label: '数据区初始化' },
        { key: '0103', label: '恢复至出厂配置' },
        {
          key: '0104',
          label: '参数（除与系统主站通信有关的）及全体数据区初始化'
        }
      ],
      dataForm: {
        rtuAddrs: [],
  ...
  methods: {
    ...
    wrapperObj(item) {
      ...
      if (this.dataForm.fn === '0403') {
        ...
        sendParam.subMins = (
          (new Date().getTime() - new Date(data.rtuDate).getTime()) /
          (1000 * 60)
        ).toFixed(2)
      }
      return sendParam
