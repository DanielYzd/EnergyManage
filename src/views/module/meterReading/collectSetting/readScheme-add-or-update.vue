<template>
	<el-dialog custom-class="dialogcommon" :title="isAdd ? '新增' : '修改'" :close-on-click-modal="false"
		...
		<el-form style="width:400px;" :model="dataForm" :inline="true" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
			label-width="140px" size="small">
			<el-form-item label="采集方案" prop="schemeid">
				<el-select v-model="dataForm.schemeid" placeholder="请选择采集方案" @change="schemeChangeHandler"
					:disabled="!isAdd">
					<el-option v-for="item in schemeList" :key="item.schemeid" :label="item.schemedesc"
						:value="item.schemeid"></el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="数据类型" prop="datatype">
				<el-select v-model="dataForm.datatype" placeholder="请选择数据类型" :disabled="!isAdd">
					<el-option v-for="item in datatypeList" :key="item.datatype" :label="item.datadesc"
			...
			<el-form-item label="采集周期类型" prop="collectcyctype">
				<el-select v-model="dataForm.collectcyctype" placeholder="请选择采集周期类型">
					<el-option v-for="item in collectcyctypeList" :key="item.value" :label="item.key"
			...
			<el-form-item label="采集参考时间" prop="collectreftime">
				<el-date-picker v-model="dataForm.collectreftime" value-format="yyyy-MM-dd HH:mm:ss" align="right"
...
export default {
  ...
  methods: {
    ...
    dataFormSubmit() {
      this.$http.ajaxPost(
        `/hes/schemeDetail/${this.isAdd ? 'insert' : 'update'}`,
        this.dataForm,
        data => {
          this.$message({
            message: '操作成功',
