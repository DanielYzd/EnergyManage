<template>
  <el-dialog
    ...
    <el-form :model="dataForm" ref="dataForm" @keyup.enter.native="dataFormSubmit()" size="small" label-width="140px">
	        ...
	        <el-select v-model="dataForm.jobType" placeholder="任务名称" style="width: 100%;" @change="handlerChange">
			      <el-option v-for="item in jobTypeList" :key="item.value" :label="item.key" :value="item.value"></el-option>
			    </el-select>
	      </el-form-item>
  ...
  export default {
    data () {
      return {
        ...
        jobTypeList : [{'value' :0,'key' :'计算费控用户扣费金额'},{'value' :1,'key' :'计算计量点电量'}]
      }
    },
    components: {
    	hlDatePicker
