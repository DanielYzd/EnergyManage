		...
		<el-form ref="form" :inline="true" :model="dataForm" @keyup.enter.native="getDataList()" size="small">
	    	...
	    	<el-form-item label="事件状态">
	    		<el-select v-model="dataForm.status" placeholder="操作类型" clearable style="width:220px">
			      <el-option v-for="item in statusList" :key="item.value" :label="item.key" :value="item.value"></el-option>
			    </el-select>
	    	</el-form-item>
	    	<el-form-item label="事件类型">
	    		<el-select v-model="dataForm.eventitemid" placeholder="发送状态" clearable style="width:220px">
			      <el-option v-for="item in eventitemidList" :key="item.eventitemid" :label="item.disc" :value="item.eventitemid"></el-option>
			    </el-select>
	    	</el-form-item>
		...
		data(){
			return {
				dataForm : {
					...
					datatime : [tool.formatDate(tool.addDay(new Date(),-15),'yyyy-MM-dd'),tool.formatDate(tool.addDay(new Date(),1),'yyyy-MM-dd')],
					hm :'',
					telephone : '',
					status : null,
					eventitemid : null
