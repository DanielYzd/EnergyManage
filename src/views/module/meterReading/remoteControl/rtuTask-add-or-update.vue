  	...
  	<el-dialog
    	...
    	<el-form :model="dataForm" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="140px" size="small">
    		...
    		<el-form-item label="任务模板" prop="schemeid">
        		<el-select v-model="dataForm.schemeid" placeholder="请选择任务模板" style="width:80%" @change="schemeChangeHandler">
			      	<el-option v-for="item in schemeList" :key="item.schemeid" :label="item.disc" :value="item.schemeid"></el-option>
			    </el-select>
      		</el-form-item>
	...
	export default{
		...
		methods : {
			...
			dataFormSubmit (isSend) {
				this.$http.ajaxGet(`/pob/rtu/select/${this.dataForm.commaddress}`,{},(data)=>{
					if(!data.rtu || !data.rtu.rtuid){
						this.$message.error(this.dataForm.commaddress+"的终端不存在，请输入正确的终端地址！");
						return ;
					}
	              	var params = [{
			...
			doSave(isSend,params){
				this.$http.ajaxPost(`/remote/task/${this.isAdd ? 'insert' : 'update'}`,params,(data)=>{
