<template>
  ...
  >
        ...
        <template v-if="dataForm.appModeid === 0">
          <el-form-item label="应用场景类型">
            <el-radio-group
              ...
              v-for="(type, index) in appTypeList"
              key="1"
    ...
    <el-button style="margin-top: 12px;" @click="back" v-if="active > 0"
      >上一步</el-button
    >
  ...
  methods: {
    ...
    getSelectRegion(data) {
      ...
      this.url = this.$http.adornUrl(
        `/pob/insertTemplate/batchImport?appType=${
          this.dataForm.appType
        }&regionid=${this.dataForm.regionid}&token=${this.$cookie.get('token')}`
      )
    },
