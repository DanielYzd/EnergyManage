	...
	<div>
		<el-form ref="form" :inline="true" :model="dataForm" @keyup.enter.native="getDataList()" size="small">
			...
			<el-form-item label="操作时间">
				<el-date-picker v-model="dataForm.operateDate" type="daterange" style="width: 220px;"
			...
			<el-form-item label="操作类型">
				<el-select v-model="dataForm.operateType" placeholder="操作类型" clearable style="width:220px">
					<el-option v-for="item in optTypeList" :key="item.value" :label="item.key" :value="item.value">
					</el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="操作结果">
				<el-select v-model="dataForm.operateResult" placeholder="发送状态" clearable style="width:220px">
					<el-option v-for="item in optStatusList" :key="item.value" :label="item.key" :value="item.value">
					</el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="操作途径">
				<el-select v-model="dataForm.optChannel" placeholder="操作途径" clearable style="width:220px">
					<el-option v-for="item in optChannelList" :key="item.value" :label="item.key" :value="item.value">
					</el-option>
				</el-select>
			</el-form-item>
		...
		<table
			style="width: 100%;height: 30px;line-height: 30px; text-align: center;color:#82848A ; border: 1px solid #15a193;margin-bottom: 10px;font-weight: 600;">
			<tr>
				<td>操作总次数 ：{{ sum }} 次</td>
