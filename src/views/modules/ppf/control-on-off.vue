<template>
    ...
    <el-card class="box-card" style="width: 40%;min-width: 180px;">
  		<el-form :model="dataForm" label-width="100px" label-position="left" size="small">
			...
			<el-form-item label="手机号码">
	    		<el-input v-model="dataForm.telephone"  :disabled="true" style="width: 80%;"></el-input>
	  		</el-form-item>
			<el-form-item label="所有表计">
	    		<el-radio-group v-model="selectedMeter.pointid" @change="pointChange">
          			<el-radio style="width: 80%;display: block;margin-top: 10px;margin-left: 30px;" v-for="(meter, index) in meterList" :label="meter.pointid" :key="index">{{ meter.disc }}</el-radio>
        		</el-radio-group>
	  		</el-form-item>
    	</el-form>
	</el-card> 
    <el-card class="box-card" style="flex:1;margin-left: 10px;">
    	<div>当前开关状态：
		  <el-button type="success" v-if="selectedMeter.status===0" circle>开</el-button>
		  <el-button type="info" v-if="selectedMeter.status===1" circle>关</el-button>
    	</div>
    	<div class="singleMeter">
    		<div class="meterStatus">
    			<icon-svg  v-if="selectedMeter.status===0" class="meterOn" name="lightOn"></icon-svg>
    			<icon-svg v-if="selectedMeter.status===1" name="lightOff"></icon-svg>
    		</div>
    		<div class="meterAddress"></div>
	    	<el-button-group class="meter-button" style="margin-top: 50px;">
			  <el-button type="success" @click="controlHander(0)"  :loading="isRunning">合闸</el-button>
			  <el-button type="info" @click="controlHander(1)"  :loading="isRunning">拉闸</el-button>
			</el-button-group>
    	</div>
	</el-card> 
	</div>
	...
	export default{
		data(){
			return{
				visible : false,
				isRunning :false ,
				meterTypeList : [{key : '电',value : 0},{key : '水',value : 1},{key : '气',value : 2}],//,{key : '热',value : 3}
				dataForm : {
					hm:'',
					doorplate:'',
					telephone:'',
		...
		methods:{
			init(customerInfo){
				...
				this.$nextTick(()=>{
					this.$http.ajaxGet(`/pob/meter/customerMeters/${this.dataForm.customerid}`,{},(resOk)=>{
	    				if(resOk.code===0){
			...
			pointChange(value){
				this.meterList.some(item => {
					if(item.pointid === value){
						this.selectedMeter.status = item.useenergyStatus !== SWITCH_OFF ? SWITCH_ON : SWITCH_OFF
						return true;
					}
				})
			},
			...
			controlHander(value){
				...
				this.$http.ajaxGet(url,params,(resOk)=>{
