<template>
    ...
    <el-card class="box-card" >
  		<el-form :model="dataForm" label-width="120px" label-position="right" size="small">
			...
			<el-form-item label="手机号码">
	    		<el-input v-model="dataForm.telephone"  style="width: 60%;" :disabled="disabledFlag"></el-input>
			...
			<el-form-item label="调整后账户金额" v-if="optType===2">
    			<el-input-number style="width: 200px;" v-model="dataForm.adjustCash"  :step="50"  :max="50000"></el-input-number>
			</el-form-item>
	    	<el-form-item label="调剂原因" v-if="optType===2">
	    		<el-select v-model="dataForm.adjustType" placeholder="调剂原因" clearable>
			      <el-option v-for="item in adjustTypeList" :key="item.value" :label="item.key" :value="item.value"></el-option>
			    </el-select>
	    	</el-form-item>
			...
			<el-form-item label="缴费金额" v-if="optType===0">
    			<el-input-number style="width: 200px;" v-model="dataForm.czje"  :step="50":min="0" :max="50000"></el-input-number>
			</el-form-item>
			<el-form-item>
    			<el-button @click="visible = false">取消</el-button>
  				<el-button :loading="loading" type="primary" @click="dataFormSubmit()">{{optType===1 ? '发送短信':'确定'}}</el-button>
			</el-form-item>
    		</el-form>
	</el-card>
   </el-dialog>
	...
	export default{
		data(){
			return{
				...
				loading : false,
				optType : 0,
				adjustTypeList : [{value:0,key : '新表预置'},{value:1,key : '换表追补'},{value:2,key : '销户退补'},{value:3,key : '其他原因'}],
				dataForm : {
					customerid :0,
					doorplate : '',
					hm:'',
		...
		methods:{
			...
			toPay(){
				this.$http.ajaxGet('/ppf/customer/recharge',{
			...
			toSendSms(){
				this.$http.ajaxPost('/ppf/customer/lessMoneySms',[this.dataForm.customerid],(resOk)=>{
	    				this.$message({
		                  message: '发送成功',
		                  type: 'success',
		                  duration: 1500,
			...
			toAdjust(){
				...
				this.$http.ajaxGet('/ppf/customer/adjust',params,(resOk)=>{
