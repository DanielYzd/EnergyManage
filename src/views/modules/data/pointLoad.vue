		...
		<el-form :inline="true" label-width="80px" label-position="left"  size="small">
			<el-form-item label="所有表计">
	    		<el-select v-model="dataForm.selectedPointid" style="width: 130px;">
          			<el-option  @change="pointChange(meter.pointid,meter.type)" 
          				v-for="(meter, index) in meterList" :value="meter.pointid" :key="meter.index"
          				:label="meter.disc"></el-option>
        		</el-select>
        	</el-form-item>
        	<el-form-item label="查询时间">
	    		<el-date-picker v-model="dataForm.datatime" @change="onPick" type="daterange"  
	    			style="width: 230px;" value-format="yyyy-MM-dd" format="yyyy-MM-dd"
...
<script>
	...
	export default{
		data(){
			return{
				...
				dataForm : {
					...
					selectedPointType : 0,
					schemeid : 2 ,
          			datatime : [tool.formatDate(tool.addDay(new Date(),-15),'yyyy-MM-dd'),
          			tool.formatDate(new Date(),'yyyy-MM-dd')],
          			chartType : 0
				},
				meterList : [],
				tbCols : [{prop:"datatime", label:"时间",width:'100'},
		...
		computed:{
			...
			endDate(){
				return tool.formatDate(tool.addMonth(this.dataForm.datatime,1),'yyyy-MM-dd')
			}
		},
		activated () {
	      this.init();
		...
		methods:{
			init(){
				...
				if(params.datatime){
					...
					this.dataForm.datatime  = [tool.formatDate(tmpDate,'yyyy-MM-dd 00:00:00')
			...
			loadData(){
				var intervalDays = tool.getIntervalDays(new Date(this.dataForm.datatime[0]),new Date(this.dataForm.datatime[1]))
				if(intervalDays>60){
			...
			loadCharts(chartType){
	    		...
	    		this.$http.ajaxGet(this.tbUrl,param,(res)=>{
	    			...
	    			let legendData = ['有功功率','无功功率'],yAxisUnit = '单位 : 千瓦(kW)';
	    			if(chartType === 1){
	    			...
	    			loadLineChart({
						...
						grid : true,
						yAxisUnit : yAxisUnit,
						title : this.dataForm.hm + this.chartList[this.dataForm.chartType] + "曲线",
						titlePostion : 'center',
						legend : legendData ,
						meterType : 3
			        })
			...
			queryBmData(){
		        ...
		        this.$router.push({ name: name,params: { customerid : this.dataForm.customerid ,pointid : this.dataForm.selectedPointid ,datatime :this.dataForm.datatime[0] , schemeid : 2  }})
		    }
		}
	}
</script>
