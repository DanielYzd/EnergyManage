...
<template>
	<div>
		<el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()" size="small">
			...
			<el-form-item>
			    <el-select v-model="dataForm.type"  placeholder="请选择表类型" style="width:80px;"> <!--multiple-->
				    <el-option
	    	...
	    	<hl-date-picker v-if="dataForm.schemeid === 1 ||dataForm.schemeid === 2 " v-model="dataForm.datatime" ></hl-date-picker>
			...
			<el-date-picker v-if="dataForm.schemeid === 3" v-model="dataForm.datatime" type="month" placeholder="选择月份"  style="width: 130px;"
				value-format="yyyy-MM" format="yyyy-MM" size="small">
			</el-date-picker>
    		<el-date-picker v-model="dataForm.dates" type="daterange" v-if="dataForm.schemeid === 4" 
		...
		<hltable v-bind:tburl="tbUrl" v-bind:tbcols="tbCols" ref="dataTable" v-bind:tbconfig="tbConfig" @dataDetail="energyDetail"/>
	</div>
</template>
...
<script>
	...
	export default{
		data(){
			return {
				...
				dataForm:{
					...
					schemeid : 2,
					dataFilterType : 1 ,
					dates : [tool.formatDate(tool.addDay(new Date(),-15),'yyyy-MM-dd'),tool.formatDate(tool.addDay(new Date(),1),'yyyy-MM-dd')],
					hm : '',
					telephone : '',
					commaddress : ''
				}
	    ...
	    computed: {
	      	...
	      	tbCols (){
	      		let cols =   [ {prop:"regionName", label:"所属区域",'min-width':'120'},
	      		...
	      		if(this.dataForm.type ==1 || this.dataForm.type === 2){
	      			...
	      			cols = cols.concat( [{prop:"rate", label:"倍率",width:'80'},
		...
		methods:{
		    ...
		    energyDetail(row){
		        ...
		        this.$router.push({ name: name,params: { customerid : row.customerid ,pointid : row.pointid ,datatime :row.datatime , schemeid :this.dataForm.schemeid  }})
		    }
