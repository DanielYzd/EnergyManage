<template>
	<el-dialog custom-class="dialogcommon" title="添加重算任务" :close-on-click-modal="false" :visible.sync="visible">
		<el-form :model="dataForm" ref="dataForm" @keyup.enter.native="dataFormSubmit()" size="small"
			label-width="140px">
			...
			<el-form-item label="表计名称" prop="pointNames">
				<el-input v-model="dataForm.pointNames" placeholder="表计名称" clearable></el-input>
			</el-form-item>
			<el-form-item label="任务名称" prop="jobType">
				<el-select v-model="dataForm.jobType" placeholder="任务名称" style="width: 100%;" @change="handlerChange">
					<el-option v-for="item in jobTypeList" :key="item.value" :label="item.key" :value="item.value">
					</el-option>
				</el-select>
			</el-form-item>
			<hl-date-picker label="开始时间" v-model="dataForm.datatime"></hl-date-picker>
			<hl-date-picker label="结束时间" v-model="dataForm.datatime2" disabled="true"></hl-date-picker>
			<el-form-item>
