		...
		<el-form :model="dataForm" :inline="true" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
			...
			<template v-if="dataForm.type === 4">
				<el-form-item label="开关回路标识" v-if="dataForm.type === 4">
					<el-select v-model="dataForm.loopMark" placeholder="请选择" style="width: 200px" disabled>
						<el-option v-for="item in loopMarkList" :key="item.value" :label="item.key" :value="item.value">
						</el-option>
					</el-select>
				</el-form-item>
				<el-form-item>
			...
			<template v-if="dataForm.type === 5">
				<el-form-item label="操作类型">
					<el-select v-model="dataForm.optModeType" placeholder="请选择" style="width: 200px"
						v-if="dataForm.type === 5">
						<el-option v-for="item in optModeTypeList" :key="item.value" :label="item.key"
    ...
    dataFormSubmit(optModeValue) {
      ...
      if (this.dataForm.type === 4) {
        params = Object.assign(params, {
          loopMark: this.dataForm.loopMark,
          switchType: optModeValue
        })
      } else {
        params = Object.assign(params, {
          mode: this.dataForm.optModeType,
          modeParams: optModeValue
        })
      }
