    ...
    :visible.sync="visible">
    	<el-form :model="dataForm" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="140px" size="small">
	  		<el-form-item label="终端地址" prop="rtuAddr">
	  			...
	  			<!--<el-select v-model="dataForm.rtuid"
					    filterable remote clearable  reserve-keyword placeholder="请输入终端地址"
			...
			<el-form-item label="表序号" prop="numberid">
	        	<el-input-number v-model="dataForm.numberid" require :min="1" :max="1000" label="请输入表序号"></el-input-number>
			</el-form-item>
			<el-form-item v-if="this.flag === 1">
				<label>同一终端下存在相同表地址，会导致该表地址的数据抄不到，可使用此清除命令，去掉表地址重复的表序号！</label>
			</el-form-item>
		    <el-form-item>
		    	...
		    	this.$http.ajaxGet('/pob/rtu/list',params,(data) => {
	              	if (data && data.code === 0) {
	              		...
	              		if(isCallback && this.rtuList.length > 0){
	              			this.$emit('sendAction',this.flag,this.rtuList[0].rtuid,this.dataForm.rtuAddr,this.dataForm.numberid)
	              		}
	              	}
		        })
		    },
