<template>
	<div>
		<el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()" size="small">
			<region-select label="所属区域" v-model="dataForm.regionName" @getRegion="getSelectRegion" clearable></region-select>
	      	<el-form-item>
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('pob:rtu:save')" :disabled="selectedItem!=0" type="primary" @click="addOrUpdateHandle()" icon="el-icon-circle-plus-outline">新增</el-button>
        <el-button v-if="isAuth('pob:rtu:delete')" :disabled="selectedItem!=0||dataListSelections.length <= 0" type="danger" @click="deleteHandle()" icon="el-icon-delete">删除</el-button>
        <el-button v-if="isAuth('pob:rtu:save')" :disabled="selectedItem==0||selectedItem==2" type="primary" @click="bacthCallHandle()">召读</el-button>
    ...
    <hltable v-bind:tburl="url" v-bind:tbcols="cols" ref="rtuTable" v-bind:tbstyle="tbstyle" v-bind:tbconfig="tbconfig" 
    	@addOrUpdateHandle="addOrUpdateHandle" @deleteHandle="deleteHandle" @callHandle="callHandle" @sendHandle="sendHandle"
	...
	export default {
	    ...
	    data () {
	      return {
	      	selectedItem : 0,
	      	taskType : 0 ,// 0 召读；1下发
	      	itemList:[{ value:0,label:'基本参数'},{value:1,label:'通讯参数'}
	      		,{value:2,label:'终端复位'},{value:3,label:'终端对时'}],
	      	fn : '',
	      	tbstyle:{
	      		width: '100%'
	      	},
	    ...
	    methods: {
	    	...
	    	selectedChange(value){
	    		if(value===0){
	    			...
	    			this.cols=[{prop:"commaddress", label:"终端地址",showRadio:false}
	    				,{prop:"protocolidDesc", label:"规约类型"}
	    				,{prop:"readschemeidDesc", label:"抄表方案"}
	    				,{prop:"curstatusDesc", label:"终端状态",tag: true,getColor:function(row){
	    			...
	    			this.cols=[{prop:"commaddress", label:"终端地址"}
	    				,{prop:"curstatusDesc", label:"终端状态",tag: true,getColor:function(row){
		      				return row.curstatus === 1 ?'success' : 'warning'
		      			}}
	        ...
	        deleteHandle(row){
		        ...
		        this.$confirm(`确定对[终端地址为${rtuAddres}]进行<b style="color:red">“${id ? '删除' : '批量删除'}”</b>操作?`, '提示', {
	        ...
	        sendHandle(row){
	        	this.rtuSendVisible = true;
	          	this.$nextTick(() => {
	          		this.$refs.rtuSend.init([{rtuid: row.rtuid,rtuAddr : row.commaddress}],this.fn);
		        })
	        },
	        sendHandleAction(sendRtuData){
	        	var vm = this;
	        	var tableData = this.$refs.rtuTable.tableData;
	        	sendRtuData.forEach((rtuData,i)=>{
	        		tableData.some((item,index)=>{
	        ...
	        convertObj(data,item,index){
        		...
        		if(this.fn === '0531'){
        			if(this.taskType === 0){
    					var tmp = result.split(';')
    					var callDate = '20'+tmp[0].substr(2,2)+'-'+
    										tmp[1].substr(2,2)+'-'+
    										tmp[2].substr(2,2)+' '+
    										tmp[3].substr(2,2)+':'+
    										tmp[4].substr(2,2)+':'+
    										tmp[5].substr(2,2);
    					item.sysDate = this.formatDate(new Date(),"yyyy-MM-dd hh:mm:ss");
    					item.subMins = ((new Date().getTime() - new Date(callDate).getTime()) / (1000*60)).toFixed(2)
