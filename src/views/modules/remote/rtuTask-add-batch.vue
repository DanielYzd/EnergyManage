  	...
  	<el-dialog title="批量增加"
    	...
    	<el-form-item label="任务模板">
    		<el-checkbox-group v-model="dataForm.schemeids">
				<el-checkbox v-for="item in schemeList" :key="item" :label="item">{{item}}</el-checkbox>
			</el-checkbox-group>
      	</el-form-item>
	...
	export default{
		data(){
			return{
		        ...
		        schemeList : ['正有、反有、正无、反无总电能示值曲线','负荷曲线','一类数据当前总尖峰平谷电能示值曲线']
			}
		},
		mounted(){
			this.init()
		...
		methods : {
			...
			doSave(params){
				this.$http.ajaxGet(`/remote/task/batchAdd`,params,(data)=>{
