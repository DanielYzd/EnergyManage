    ...
    >
      <el-form-item label="终端地址" prop="commaddress">
        ...
        <el-button
          @click="
            () => {
              dataForm.commaddress = getRtuAddress()
            }
          "
          icon="el-icon-back"
        ></el-button>
      ...
      <el-form-item
        label="采集方案"
        prop="readschemeid"
        v-if="dataForm.type != 6"
      >
        <el-select
          v-model="dataForm.readschemeid"
          placeholder="采集方案"
          style="width: 80%;"
          clearable
        >
          <el-option
            v-for="item in readschemeList"
            :key="item.schemeid"
      ...
      <el-form-item label="高级密码" prop="passwordH">
        <el-input
          v-model="dataForm.passwordH"
          placeholder="高级密码"
        ></el-input>
      </el-form-item>
      <el-form-item label="低级密码" prop="passwordL">
        <el-input
          v-model="dataForm.passwordL"
          placeholder="低级密码"
        ></el-input>
      </el-form-item>
      <!--</el-collapse-item>-->
        ...
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/pob/rtu/${!this.dataForm.rtuid ? 'save' : 'update'}`
            ),
            method: 'post',
