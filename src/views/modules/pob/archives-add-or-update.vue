<template>
  <el-dialog
    ...
    :visible.sync="visible" @close="resetForm">
  		...
  		<el-tabs v-model="activedTabName" tab-position="left">
		    <el-tab-pane label="用户信息" name="-1">
        		...
        		<el-checkbox-group v-model="type" @change="meterTypeChange">
					    <el-checkbox v-for="item in meterTypeList" :key="item.value" :label="item.value">{{item.key}}</el-checkbox>
					  </el-checkbox-group>
      	  </el-form-item>
			  	<el-form-item label="主站费控" prop="ppfstatus">
        		<el-switch v-model="dataForm.ppfstatus" @change="ppfSwitchChange" active-color="#13ce66" active-text="开启"></el-switch>
      	  </el-form-item>
		    </el-tab-pane>
		    <el-tab-pane label="计费参数" v-if="dataForm.isppf" name="4">
		    		<el-form-item label="计费开始日期" style="display: inline-block;" prop="starttime">
			        <el-date-picker v-model="dataForm.starttime" style="width: 160px;" type="date" value="yyyy-MM-dd" placeholder="开户日期" value-format="yyyy-MM-dd 00:00:00"></el-date-picker>
	      		</el-form-item>
						<el-form-item label="计费结束日期" prop="checkOutTime" style="display: inline-block;">
				      		<el-date-picker v-model="dataForm.checkOutTime" value-format="yyyy-MM-dd" align="right" type="date" placeholder="选择日期" style="width: 160px;">
		    					</el-date-picker>
		    		</el-form-item>
      			<el-form-item label="结算周期" prop="jszq" style="display: inline-block;">
			        ...
			        <el-input-number v-model="dataForm.khje" :disabled="!isCanModify" :min="0" :max="50000" label="请输入预充金额" style="width: 140px;"></el-input-number>
	      			<label>元</label>
	    			</el-form-item>
	    			<el-form-item label="催费金额" style="display: inline-block;" prop="bjje">
			        <el-input-number v-model="dataForm.bjje" :min="0" :max="5000" label="请输入催费金额"style="width: 160px;"></el-input-number>
	      			<label>元</label>
	    			</el-form-item>
	    			<br>
	    			<el-form-item label="自动拉闸" style="display: inline-block;" prop="autoControl">
			        <el-switch v-model="dataForm.autoControl" active-color="#13ce66" active-text="开启" style="width: 160px;"></el-switch>
	      		</el-form-item>
		      	...
		      	<el-form-item label="电价" prop="unitPriceEle" style="display: inline-block;">
			        <el-select v-model="dataForm.unitPriceEle" placeholder="请选择电价"style="width: 160px;">
					      <el-option v-for="item in eleUnitPriceList" :key="item.priceid" :label="item.disc" :value="item.priceid"></el-option>
					    </el-select>
	    			</el-form-item>
			        ...
			        <el-select v-model="dataForm.unitPriceWat" placeholder="请选择水价"style="width: 160px;">
					      <el-option v-for="item in watUnitPriceList" :key="item.priceid" :label="item.disc" :value="item.priceid"></el-option>
					    </el-select>
	    			</el-form-item>
		    </el-tab-pane>
		    	...
		    	<div style="display:flex;">
		      	  <el-tabs v-model="activedTabName_0" type="border-card" style="flex:1;max-width: 750px;" closable 
		      	  	@tab-remove="removeMeter($event,0)">
						    <el-tab-pane v-for="(meter, index) in dataForm.eleMeters"
								    :label="(index + 1)+'号电表'"
								    :key="meter.key" :name="index.toString()">
									      ...
									      <el-form-item label="PT(电压互感器)" prop="ptid">
									        ...
									        <el-select v-model="meter.energyType" clearable  placeholder="能耗用途" style="width: 100%;">
											      <el-option v-for="item in energyEleTypeList" :key="item.value" :label="item.key" :value="item.value"></el-option>
											    </el-select>
									      </el-form-item>
						    </el-tab-pane>
						  </el-tabs>
  	  				<div style="width:100px;">
					    	...
					    	<el-button  v-if="isAuth('pob:meter:save')" @click.prevent="addMeter(0)"  icon="el-icon-circle-plus"></el-button>
					    	</el-tooltip>
				  		</div>
				  </div>
		    </el-tab-pane>
		    	...
		    	<div style="display:flex;">
		      	  <el-tabs v-model="activedTabName_1" type="border-card" style="flex:1" closable 
		      	  	@tab-remove="removeMeter($event,1)">
						    <el-tab-pane v-for="(meter, index) in dataForm.watMeters"
								    :label="(index + 1)+'号水表'" :name="index.toString()"
								    :key="meter.key">
									    	<el-form-item label="表计名称" prop="disc" style="margin-bottom: 15px;">
											    <el-input v-model="meter.disc"  placeholder="请输入电表名称"></el-input>
											  </el-form-item>
									        ...
									        <el-select v-model="meter.energyType" placeholder="能耗用途" style="width: 100%;">
											      <el-option v-for="item in energyWatTypeList" :key="item.value" :label="item.key" :value="item.value"></el-option>
											    </el-select>
									      </el-form-item>
						    </el-tab-pane>
						  </el-tabs>
  	  				<div style="width:100px;">
				    		<el-tooltip effect="dark" placement="top-start">
	    						<div slot="content">增加</div>
					    	<el-button  v-if="isAuth('pob:meter:save')" @click.prevent="addMeter(1)"  icon="el-icon-circle-plus"></el-button>
					    	</el-tooltip>
				  		</div>
				  </div>
		    </el-tab-pane>
		    	...
		    	<div style="display:flex;">
		      	  <el-tabs v-model="activedTabName_2" type="border-card" style="flex:1" closable 
		      	  	@tab-remove="removeMeter($event,2)">
						    <el-tab-pane v-for="(meter, index) in dataForm.gasMeters"
								    :label="(index + 1)+'号气表'" :name="index.toString()"
								    :key="meter.key">
									    	<el-form-item label="表计名称" prop="disc" style="margin-bottom: 15px;">
											    <el-input v-model="meter.disc"  placeholder="请输入电表名称"></el-input>
											  </el-form-item>
									    	...
									    	<el-form-item label="能耗用途" prop="energyType">
									        <el-select v-model="meter.energyType" placeholder="能耗用途" style="width: 100%;">
											      <el-option v-for="item in energyGasTypeList" :key="item.value" :label="item.key" :value="item.value"></el-option>
											    </el-select>
									      </el-form-item>
						    </el-tab-pane>
						  </el-tabs>
  	  				<div style="width:100px;">
				    		<el-tooltip effect="dark" placement="top-start">
	    						<div slot="content">增加</div>
					    	<el-button  v-if="isAuth('pob:meter:save')"  @click.prevent="addMeter(2)"  icon="el-icon-circle-plus"></el-button>
					    	</el-tooltip>
				  		</div>
				  </div>
		    </el-tab-pane>
		    	...
		    	<div style="display:flex;">
		      	  <el-tabs v-model="activedTabName_3" type="border-card" style="flex:1" closable 
		      	  	@tab-remove="removeMeter($event,3)">
						    <el-tab-pane v-for="(meter, index) in dataForm.hotMeters"
								    :label="(index + 1)+'号热表'" :name="index.toString()"
								    :key="meter.key">
									    	<el-form-item label="表计名称" prop="disc" style="margin-bottom: 15px;">
											    <el-input v-model="meter.disc"  placeholder="请输入电表名称"></el-input>
											  </el-form-item>
									    	...
									    	<el-form-item label="能耗用途" prop="energyType">
									        <el-select v-model="meter.energyType" placeholder="能耗用途" style="width: 100%;">
											      <el-option v-for="item in energyHotTypeList" :key="item.value" :label="item.key" :value="item.value"></el-option>
											    </el-select>
									      </el-form-item>
						    </el-tab-pane>
						  </el-tabs>
  	  				<div style="width:100px;">
				    		<el-tooltip effect="dark" placement="top-start">
	    						<div slot="content">增加</div>
					    	<el-button  v-if="isAuth('pob:meter:save')"  @click.prevent="addMeter(3)"  icon="el-icon-circle-plus"></el-button>
					    	</el-tooltip>
				  		</div>
				  </div>
		    </el-tab-pane>
  ...
  export default {
    data () {
      ...
      return {
        ...
        oldType :[0],
        isCanModify : true ,
        schemeidList : [{schemeid : 2,disc:'每天'},{schemeid : 3,disc:'每月'}],
        dataForm: {
          customerid: 0,
          disc :'',
        ...
        energyEleTypeList : [{'value' :1,'key' :'照明与插座用电'},{'value' :2,'key':'空调用电'},{'value' :3,'key':'动力用电'},{'value' :4,'key':'特殊用电'}],
        energyWatTypeList : [{'value' :5,'key':'厨卫生活用水'},{'value' :6,'key':'道路景观用水'}],
...
//        ]
    ...
    methods: {
    	...
    	setInitData(){
	        	...
	        	this.dataForm.starttime = tool.formatDate(new Date(),'yyyy-MM-dd 00:00:00')
      ...
      init (id) {
        ...
        if (this.dataForm.customerid) {
            this.$http({
              url: this.$http.adornUrl(`/pob/customerUnion/info/${this.dataForm.customerid}`),
              method: 'get',
              ...
              if (data && data.code === 0) {
				        ...
				        if(ppfInfo && ppfInfo.ppfstatus === PPF_STATUS_ON){
				        	...
				        	this.dataForm.checkOutTime = ppfInfo.checkOutTime
				  				this.dataForm.autoControl = ppfInfo.autoControl === PPF_AUTOCONTROL_ON
      ...
      loadUnitPrice(){
	          ...
	          this.dataForm.unitPriceWat = null
	          this.dataForm.unitPriceGas = null
      	this.$http.ajaxGet('/ppf/unitprice/list/',{
      ...
      removeMeter(index,meterType){
      	...
      	if(!tMeters[index].pointid){
	      	...
	      	this.$confirm(`确定对[表名为${tMeters[index].disc}]进行<b style="color:red">删除</b>操作?`, '提示', {
      ...
      getDefaultMeterObj(meterType,numbers){
      	...
      	return {
      		...
      		disc: this.dataForm.disc + this.getMeterTypeName(meterType)+ (numbers?numbers:'1'),
      ...
      // 表单提交
      dataFormSubmit () {
      	if(this.dataForm.ppfstatus && this.dataForm.eleMeters.length === 0 && this.dataForm.watMeters.length === 0
      		 && this.dataForm.gasMeters.length === 0 && this.dataForm.hotMeters.length === 0){
      		 	this.$message.error("用户下没有关联任何表计，不能开启主站费控")
      			return ;
      	}
      	if(this.dataForm.ppfstatus && this.dataForm.unitPriceEle === null && this.dataForm.unitPriceWat === null){
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/pob/customerUnion/${!this.dataForm.customerid ? 'saveUnion' : 'updateUnion'}`),
              method: 'post',
