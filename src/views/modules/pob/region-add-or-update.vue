    	...
    	methods: {
      		init (id,parentid,layer) {
        		...
        		if(this.dataForm.regionid){
        			this.$http.ajaxGet(`/pob/region/info/${this.dataForm.regionid}`,{},(data)=>{
       		...
       		dataFormSubmit(){
       			this.$http({
	              	url: this.$http.adornUrl(`/pob/region/${!this.dataForm.regionid ? 'add' : 'update'}`),
	              	method: 'post',
